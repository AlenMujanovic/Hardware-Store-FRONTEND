...
export default class Cart extends React.Component {
    ...
    private calculateSum(): number {
        ...
        if (this.state.cart === undefined) {
            ...
            for (const item of this.state.cart?.cartArticles) {
                sum +=
                    item.article.articlePrices[item.article.articlePrices.length - 1].price * item.quantity;
            }
        }
    ...
    private makeOrder() {
        api("/api/user/cart/makeOrder/", "post", {}).then((res: ApiResponse) => {
            if (res.status === "error" || res.status === "login") {
                this.setStateCount(0);
                this.setStateCart(undefined);
                return;
                        ...
                        <Table hover size="sm">
                            ...
                            <tbody>
                                {this.state.cart?.cartArticles.map((item) => {
                                    const price = Number(
                                        item.article.articlePrices[item.article.articlePrices.length - 1]
                                            .price
                                    ).toFixed(2);
                                    const total = Number(
                                        item.article.articlePrices[item.article.articlePrices.length - 1]
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <FontAwesomeIcon
                                                    icon={faMinusSquare}
                                                    onClick={() =>
                                                        this.removeFromCart(item.article.articleId)
                                                    }
                                                />
                                            </td>
                                        </tr>
