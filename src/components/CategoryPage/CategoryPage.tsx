...
export default class CategoryPage extends React.Component<CategoryPageProperties> {
    ...
    private showSubcategories() {
        ...
        return <Row>{this.state.subcategories?.map(this.singleCategory)}</Row>;
    }
    ...
    private showArticles() {
        if (this.state.articles?.length === 0) {
            return <div>There are no articles in this category.</div>;
        }

        return <Row>{this.state.articles?.map(this.singleArticle)}</Row>;
    }
            ...
            <Col lg="4" md="6" sm="6" xs="12">
                <Card className="mb-3">
                    ...
                    <Card.Body>
                        ...
                        <Card.Text>Price: {Number(article.price).toFixed(2)} EUR</Card.Text>
                        <Link
    ...
    private getCategoryData() {
        api("api/category/" + this.props.match.params.cId, "get", {}).then((res: ApiResponse) => {
            if (res.status === "login") {
    ...
    getFeatures() {
        api("api/feature/values/" + this.props.match.params.cId, "get", {}).then((res: ApiResponse) => {
            if (res.status === "login") {
                return this.setLogginState(false);
            }
